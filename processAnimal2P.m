...
dataPath = 'C:\Users\JIC402\OneDrive - University of Pittsburgh\Data\AA0036\Region1';
...
for roiFigN = 1:roiFigNo
    ...
    for roiSubPlotN = 1:ROIperFig
        ...
        if curROIno <= nCell
            ...
            p1=plot(time_vector,F_high_avg(m,:))
            hold on;
            p2=plot(time_vector,F_low_avg(m,:))
            xlabel('time/s')
           
            xline(2,'--')
            hold off;
            title('ROI'+string(m));
            legend('High contrast', 'Low contrast','pure tone')
            m=m+1
        end
        clear curROIno
    end
